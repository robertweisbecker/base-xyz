...
test("custom render host merges semantics, refs, events, and theme state without a wrapper", async ({ page }) => {
	...
	await expect(host).toHaveAttribute("data-mode", "dark");
	...
	await expect(host.getByTestId("theme-context").first()).toHaveAttribute("data-resolved-mode", "dark");
});
...
test("nested MP overrides stay scoped away from the default sibling", async ({ page }) => {
	await page.goto(storyPath);
	const customAccent = await page.getByTestId("custom-accent").evaluate((element) => getComputedStyle(element).backgroundColor);
	...
	const nestedDefaultAccent = await page
		...
		.evaluate((element) => getComputedStyle(element).backgroundColor);
	const defaultDarkAccent = await page
		.getByTestId("default-dark-accent")
		.evaluate((element) => getComputedStyle(element).backgroundColor);
	...
	expect(nestedDefaultAccent).toBe(defaultDarkAccent);
	expect(bodyPortalAccent).toBe(fallbackAccent);
	expect(nestedPortalAccent).toBe(customAccent);
	await expect(page.getByTestId("nested-default-host")).toHaveAttribute("data-mode", "dark");
