import * as stylex from "@stylexjs/stylex";
import { breakpoints } from "@/styles/constants.stylex";
import {
	border,
	colors,
	fontSize,
	fontWeight,
	letterSpacing,
	lineHeight,
	radius,
	shadow,
	size,
	space,
} from "@/styles/tokens.stylex";

const activeDropTarget = ":is([data-drop-target], [data-drop-target='true'], [data-drop-target=''])";
const invalidTarget = ":is([data-invalid], [data-invalid='true'], [data-invalid=''])";

export const demoStyles = stylex.create({
	stack: {
		gap: space[5],
		display: "grid",
	},
	comparison: {
		gap: space[5],
		alignItems: "start",
		display: "grid",
		gridTemplateColumns: {
			default: "minmax(0, 1fr)",
			[breakpoints.md]: "repeat(2, minmax(0, 1fr))",
		},
	},
	panel: {
		padding: space[4],
		borderColor: colors["--border"],
		borderRadius: radius.md,
		borderStyle: "solid",
		borderWidth: border.width,
		gap: space[4],
		backgroundColor: colors["--surface"],
		display: "grid",
		minWidth: 0,
	},
	panelHeader: {
		gap: space[1],
		display: "grid",
	},
	panelFooter: {
		gap: space[3],
		alignItems: "center",
		display: "flex",
		flexWrap: "wrap",
		justifyContent: "space-between",
	},
	instructions: {
		margin: 0,
		gap: space[1],
		color: colors["--text-muted"],
		fontSize: fontSize.x1,
		lineHeight: lineHeight.x1,
	},
	instructionItem: {
		display: "list-item",
		listStylePosition: "inside",
		listStyleType: "auto",
		paddingInlineStart: space[5],
	},
	status: {
		margin: 0,
		color: colors["--text-muted"],
		fontSize: fontSize.x1,
		lineHeight: lineHeight.x1,
	},
	workArea: {
		gap: space[4],
		display: "grid",
	},
	transferGrid: {
		gap: space[3],
		alignItems: "stretch",
		display: "grid",
		gridTemplateColumns: {
			default: "minmax(0, 1fr)",
			[breakpoints.sm]: "minmax(12rem, 0.9fr) repeat(2, minmax(10rem, 1fr))",
		},
	},
	sourceTray: {
		gap: space[3],
		alignContent: "start",
		display: "grid",
		minHeight: "7rem",
	},
	card: {
		padding: space[2],
		borderRadius: radius.sm,
		gap: space[3],
		alignItems: "center",
		backgroundColor: colors["--elevated"],
		boxShadow: {
			default: shadow.sm,
			":is([data-dragging], [data-dragging='true'], [data-dragging=''])": "none",
			":hover": shadow.md,
		},
		color: colors["--text"],
		display: "flex",

		opacity: {
			default: 1,
			":is([data-dragging], [data-dragging='true'], [data-dragging=''])": 0.6,
		},
		touchAction: "none",
		transitionDuration: {
			default: "160ms",
			"@media (prefers-reduced-motion: reduce)": "0ms",
		},
		// transform: {
		// 	default: "none",
		// 	":is([data-dragging], [data-dragging='true'], [data-dragging=''])": "rotate(2deg)",
		// },
		transitionProperty: "background-color, border-color, box-shadow, opacity",
		transitionTimingFunction: "ease",
		minHeight: "3.25rem",
	},
	cardText: {
		gap: space[1],
		display: "grid",
		flexGrow: 1,
		minWidth: 0,
	},
	cardLabel: {
		overflow: "hidden",
		fontSize: fontSize.x2,
		fontWeight: fontWeight.medium,
		lineHeight: lineHeight.x2,
		textOverflow: "ellipsis",
		whiteSpace: "nowrap",
	},
	cardMeta: {
		color: colors["--text-muted"],
		fontSize: fontSize.x1,
		lineHeight: lineHeight.x1,
	},
	handle: {
		padding: 0,
		borderColor: colors["--border"],
		borderRadius: radius.sm,
		borderStyle: "solid",
		borderWidth: border.width,
		alignItems: "center",
		color: colors["--text-muted"],
		cursor: {
			default: "grab",
			":active": "grabbing",
		},
		display: "inline-flex",
		justifyContent: "center",
		justifySelf: "flex-end",
		marginInlineStart: "auto",
		height: size["control.md"],
		width: size["control.md"],
	},
	dropZone: {
		padding: space[4],
		borderRadius: radius.md,
		gap: space[2],
		alignItems: "stretch",
		backgroundColor: {
			[activeDropTarget]: colors["--accent-soft"],
			default: colors["--surface-subtle"],
			[invalidTarget]: colors["--danger-subtle"],
		},
		color: colors["--text"],
		display: "flex",
		flexDirection: "column",
		outlineColor: {
			[activeDropTarget]: colors["--accent"],
			default: colors["--border"],
			[invalidTarget]: colors["--danger"],
		},
		outlineOffset: -border.width,
		outlineStyle: "dashed",
		outlineWidth: "1px",
		minHeight: "7rem",
	},
	dropZoneLabel: {
		fontSize: fontSize.x2,
		fontWeight: fontWeight.medium,
		lineHeight: lineHeight.x2,
	},
	dropZoneDescription: {
		color: colors["--text-muted"],
		fontSize: fontSize.x1,
		lineHeight: lineHeight.x1,
	},
	invalidLabel: {
		color: colors["--text-danger"],
	},
	board: {
		gap: space[3],
		display: "grid",
		gridTemplateColumns: {
			default: "minmax(0, 1fr)",
			[breakpoints.sm]: "repeat(2, minmax(0, 1fr))",
		},
	},
	column: {
		padding: space[3],
		borderRadius: radius.md,
		gap: space[3],
		alignContent: "start",
		backgroundColor: {
			default: colors["--canvas-subtle"],
			// ":where([data-drop-target], [data-drop-target='true'], [data-drop-target=''])": colors["--highlight"],
		},
		display: "grid",
		outlineColor: {
			default: colors["--border"],
			":focus-within": colors["--accent"],
		},
		outlineOffset: -border.width,
		outlineStyle: "solid",
		outlineWidth: border.width,
		minHeight: "18rem",
	},
	columnHeader: {
		gap: space[2],
		alignItems: "center",
		display: "flex",
		justifyContent: "space-between",
	},
	columnTitle: {
		fontSize: fontSize.x2,
		fontWeight: fontWeight.semibold,
		lineHeight: lineHeight.x2,
	},
	columnCount: {
		color: colors["--text-muted"],
		fontSize: fontSize.x1,
		lineHeight: lineHeight.x1,
	},
	list: {
		margin: 0,
		padding: 0,
		gap: space[2],
		listStyle: "none",
		display: "flex",
		flexDirection: "column",
		minHeight: "12rem",
	},
	empty: {
		// padding: space[3],
		// borderColor: colors["--text"],
		// backgroundColor: colors["--highlight"],
		// borderRadius: radius.sm,
		// borderStyle: "dashed",
		// borderWidth: border.width,
		height: "100%",
		// alignItems: "center",
		// color: colors["--text-muted"],
		// display: "flex",
		// fontSize: fontSize.x1,
		// justifyContent: "center",
		// lineHeight: lineHeight.x1,
		// textAlign: "center",
		// minHeight: "4rem",
	},
	dropIndicator: {
		// borderRadius: radius.full,
		// marginInline: `-${space[2]}`,
		// width: `calc(100% + ${space[2]})`,
		// outline: `1px solid ${colors["--accent"]}`,
		// outlineColor: colors["--accent"],
		// outlineColor: "CanvasText",
		outlineStyle: {
			default: "none",
			"@media (forced-colors: active)": "solid",
		},
		outlineWidth: {
			default: 0,
			"@media (forced-colors: active)": "1px",
		},
		position: "relative",
		zIndex: 100,
		height: "0px",
		"::before": {
			borderRadius: radius.full,
			insetBlock: -2,
			insetInline: -16,
			backgroundColor: colors["--accent"],
			content: "",
			outlineColor: colors["--accent-contrast"],
			// outlineOffset: 2,
			outlineStyle: "solid",
			outlineWidth: 2,
			position: "absolute",
			minWidth: "100%",
		},
	},
	overlay: {
		boxShadow: shadow.lg,
		pointerEvents: "none",
		transform: "rotate(1deg)",
	},
	shortList: {
		gap: space[2],
		display: "grid",
		maxWidth: "24rem",
	},
	menuDemo: {
		alignItems: "start",
		columnGap: `calc(${space[8]} * 4)`,
		display: "flex",
		flexWrap: "wrap",
		position: "relative",
		rowGap: space[3],
		minHeight: "18rem",
	},
	sortableMenuPopup: {
		outlineColor: {
			default: "transparent",
			":is([data-drop-target], [data-drop-target='true'], [data-drop-target=''])": colors["--text-muted"],
		},
		outlineOffset: "4px",
		outlineStyle: "dashed",
		outlineWidth: "2px",
		minWidth: "12rem",
	},
	sortableMenuItem: {
		opacity: {
			default: 1,
			":is([data-dragging], [data-dragging='true'], [data-dragging=''])": 0.58,
		},
		outlineColor: {
			default: "transparent",
			":is([data-drop-target], [data-drop-target='true'], [data-drop-target=''])": colors["--text"],
		},
		outlineOffset: "2px",
		outlineStyle: "dashed",
		outlineWidth: "2px",
	},
	emptyMenuItem: {
		color: colors["--text-muted"],
		display: "flex",
		fontStyle: "italic",
		outlineColor: colors["--border-strong"],
		outlineOffset: "-1px",
		outlineStyle: "dashed",
		outlineWidth: "1px",
	},
	menuOverlay: {
		borderRadius: radius.md,
		overflow: "hidden",
		paddingBlock: space[2],
		paddingInline: space[2],
		alignItems: "center",
		backgroundColor: colors["--canvas"],
		boxShadow: shadow.lg,
		color: colors["--text"],
		columnGap: space[2],
		display: "flex",
		fontSize: fontSize.x2,
		letterSpacing: letterSpacing.x2,
		lineHeight: lineHeight.x2,
		outlineColor: colors["--elevated"],
		outlineOffset: "-2px",
		outlineStyle: "solid",
		outlineWidth: "2px",
		// gridTemplateColumns: "auto minmax(0, 1fr)",
		pointerEvents: "none",
		textOverflow: "ellipsis",
		whiteSpace: "nowrap",

		// minWidth: "12rem",
	},
	nestedAction: {
		justifySelf: "end",
	},
});
